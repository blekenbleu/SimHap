...
- with game running: `DataUpdate()`
	...
	- `SetCar()`, when loaded CarId `!=` requested
		- check local dictionaries
			- first, from cars cached since SimHub launch (game change)
			- else from optional read/only atlas
			- else else read/write personal
		- else call `FetchCarData()` for dictionary misses
		...
		- else call `Defaults()` *after* `FetchCarData()` fails
...
- End()
	...
	- move changed cached cars to read/write personal JSON file
...
### changes
- consolidated `SimData` methods in that source file
- likewise for `Spec`
- created a `ListDictionary` class for download server compatibility
- read, write local json e.g. to preserve changed values
- first lookup from .json, then Internet for fails, then defaults - *Done 27 May 2024*
	...
	- extended to include `Defaults()`...   
      // but no reason to save easily-regenerated `Defaults()` to JSON...  
		- *instead*,  keep the `new CarSpec` generated by `Defaults()`  
          until `DataUpdate()` invokes `S.AddCar(S.Id)`  
...
- `Runtime()` only if `EngineIgnitionOn` *30 May*  
- **refactoring**
	- when SimHub invokes `DataUpdate()` (at 60 Hz),
		- avoid invoking either `Runtime()` or `SetCar()` if `Waiting`
...
- add `BlekHapt.cs CarId` to compare for `data.NewData.CarId`
