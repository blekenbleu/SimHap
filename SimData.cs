	...
	public class SimData : NotifyPropertyChanged
	{
		...
		private void SetRPMMix()
		{
			...
			if (SHP.S.Displacement > 0.0)
			{
				...
				MixCylinder = 1.0 - Math.Max(2000.0 - CylinderDisplacement, 0.0) * Math.Max(2000.0 - CylinderDisplacement, 0.0) * 2.5E-07;
				MixDisplacement = 1.0 - Math.Max(10000.0 - SHP.S.Displacement, 0.0) * Math.Max(10000.0 - SHP.S.Displacement, 0.0) * 1E-08;
			}
			else
			{
				MixCylinder = 0.0;
			...
			MixPower = 1.0 - Math.Max(2000.0 - (SHP.S.MaxPower - SHP.S.ElectricMaxPower), 0.0) * Math.Max(2000.0 - (SHP.S.MaxPower - SHP.S.ElectricMaxPower), 0.0) * 2.5E-07;
		...
		internal void SetVehiclePerGame(PluginManager pluginManager, ref StatusDataBase db, SimHapticsPlugin shp)
		{
			...
			switch (SimHapticsPlugin.CurrentGame)
			{
				...
				case GameId.D4:
					...
					{
						SimHapticsPlugin.FetchCarData(db.CarId, null, SHP.S, db.CarSettings_CurrentGearRedLineRPM, db.MaxRpm);
						IdleRPM = 10.0 * (ushort) pluginManager.GetPropertyValue("DataCorePlugin.GameRawData.IdleRpm");
						break;
					}
					break;
				case GameId.DR2:
				...
				case GameId.WRC23:
					if (SHP.S.Id != db.CarId && SimHapticsPlugin.FailedId != db.CarId)
					{
						...
						IdleRPM = (float)pluginManager.GetPropertyValue("DataCorePlugin.GameRawData.SessionUpdate.vehicle_engine_rpm_idle");
						break;
					}
					break;
				case GameId.F12022:
				case GameId.F12023:
					...
					{
						SimHapticsPlugin.FetchCarData(db.CarId, null, SHP.S, db.CarSettings_CurrentGearRedLineRPM, db.MaxRpm);
						IdleRPM = 10.0 * (ushort) pluginManager.GetPropertyValue("DataCorePlugin.GameRawData.PlayerCarStatusData.m_idleRPM");
						break;
					}
					break;
				case GameId.Forza:
				...
				case GameId.BeamNG:
					if (SHP.S.Id != db.CarId && SimHapticsPlugin.FailedId != db.CarId)
					{
						...
						IdleRPM = (float)pluginManager.GetPropertyValue("DataCorePlugin.GameRawData.idle_rpm");
						break;
					}
					break;
				case GameId.GPBikes:
				case GameId.MXBikes:
					if (SHP.S.Id != db.CarId)
					{
						SHP.S.Id = db.CarId;
						IdleRPM = db.MaxRpm * 0.25;
						SHP.S.MaxRPM = (ushort) (0.5 + db.MaxRpm);
						SHP.S.Redline = (ushort) pluginManager.GetPropertyValue("DataCorePlugin.GameRawData.m_sEvent.m_iShiftRPM");
		...
		internal void Update(GameData data, PluginManager pluginManager, SimHapticsPlugin shp)
		{
			...
			if (IdleSampleCount < 20 && SimHapticsPlugin.FrameCountTicks % 2500000L <= 150000L)
			{
				...
				{
					IdleRPM = (IdleRPM + data.NewData.Rpms) * 0.5;
					++IdleSampleCount;
